...
const mutations = {
  ...
  async updateUserInfo(data2 = false) {
    if (data2) {
      ...
      const _id = common_vendor.Ys.getCurrentUserInfo().uid;
      this.setUserInfo({ _id }, { cover: true });
      const uniIdCo2 = common_vendor.Ys.importObject("uni-id-co", {
        customUI: true
      });
      try {
  ...
  async logout() {
    if (common_vendor.Ys.getCurrentUserInfo().tokenExpired > Date.now()) {
      try {
